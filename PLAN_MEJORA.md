...
- [x] **F2-T3 — Metas combinadas en una instrucción** · **IMPORTANTE · L** *(hecho 2026-06-10; `solve()` acepta constraints heterogéneas con orden de dependencia utilidad→no-caja→caja, detección del par en conflicto con cifras y verificación final de todas las metas. Intent nuevo `plan_compound_constraints` + regla A del prompt relajada. Caso de aceptación verificado E2E: caja promedio 5,000 USD + inventario 100k USD en una instrucción → un plan aplicado con ambas metas dentro de tolerancia. Suite: 223 passed)*
